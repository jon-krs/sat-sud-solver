...
def add_sudoku_rules(cnf):
    ...
    # Efficient Encoding: ensure there is at most one number in each cell.
    for x in range(1, 10):
        for y in range(1, 10):
            for z in range(1, 9):
                for i in range(z + 1, 10):
                    cnf.append([-encode_variable(x, y, z), -encode_variable(x, y, i), 0])
    ...
    # Extended Encoding: ensure each number appears at least once in each row.
    for y in range(1, 10):
        for z in range(1, 10):
            cnf.append([encode_variable(x, y, z) for x in range(1, 10)] + [0])

    # Extended Encoding: ensure each number appears at least once in each column.
    for x in range(1, 10):
        for z in range(1, 10):
            cnf.append([encode_variable(x, y, z) for y in range(1, 10)] + [0])

    # Extended Encoding: ensure each number appears at least once in each subgrid.
    for z in range(1, 10):
        for i in range(0, 3):
            for j in range(0, 3):
                subgrid_cells = [encode_variable(3*i + x, 3*j + y, z) for x in range(1, 4) for y in range(1, 4)]
                cnf.append(subgrid_cells + [0])
